...
// Path to the images, so that assigner knows wats wat.
...
async function runPy(script_path,callback,options=null){
    return new Promise(function(resolve, reject){
        PythonShell.run(script_path, options, function (err, results) {
                if (err) throw err;

                callback(err, results);
                resolve(results[1])  // Returned only JSON(Stringified) out of all the string received from py script
        });
    })
}

// Used to gen tag options for submission since they vary little
...
// Everything is in an async function because sync is good.
async function  main() {
...
// Used below because https://stackoverflow.com/questions/27465850/typeerror-router-use-requires-middleware-function-but-got-a-object
